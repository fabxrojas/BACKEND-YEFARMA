...
public interface InventarioRepository extends JpaRepository<IngresoProducto, Integer> {
    ...
    @Query(value = "SELECT p.id_producto, p.producto, p.codigo, m.nombre, pres.nombre, SUM(ip.cantidad_stock), ip.lote, p.precio " +
       ...
       "GROUP BY p.id_producto, p.producto, p.codigo, m.nombre, pres.nombre, ip.lote, p.precio", 
       nativeQuery = true)
